(function () {
  var dataEl = document.getElementById("ysap-data");
  if (!dataEl) return;
  ...
  function renderProfitCard(apartment) {
    setText('[data-ysap-field="badge"]', apartment.badge);
    setText('[data-ysap-field="areaGroup"]', apartment.areaGroup + " · " + apartment.legalDongLabel);
    setText('[data-ysap-field="complexName"]', apartment.complexName);
    setText(
      '[data-ysap-field="profitSentence"]',
    ...
    setText('[data-ysap-field="gain"]', formatEok(apartment.estimatedGainManwon));
    setText('[data-ysap-field="gainRate"]', formatPercent(apartment.estimatedGainRate));
    setText('[data-ysap-field="latest"]', formatEok(apartment.latestTradePriceManwon));
    setText('[data-ysap-field="latestMeta"]', apartment.latestTradeDate + " · " + apartment.latestTradeArea);
    setText('[data-ysap-field="low"]', formatEok(apartment.fiveYearLowPriceManwon));
    setText('[data-ysap-field="development"]', apartment.developmentNote);
    setText('[data-ysap-field="note"]', apartment.note);
  }

  function renderTable(sortKey) {
    var tbody = document.querySelector("[data-ysap-table-body]");
    if (!tbody) return;
    tbody.textContent = "";
  ...
  function renderBars(sortKey) {
    ...
    getSortedRows(sortKey).forEach(function (row) {
      ...
      bars.className = "ysap-bar-row__bars";
      var latest = document.createElement("span");
      latest.className = "ysap-bar ysap-bar--latest";
      ...
      low.className = "ysap-bar ysap-bar--low";
  ...
  function updateTabs() {
    document.querySelectorAll(".ysap-tab").forEach(function (button) {
