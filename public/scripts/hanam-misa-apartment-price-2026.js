(function () {
  var dataEl = document.getElementById("hmap-data");
  if (!dataEl) return;
  ...
  function renderProfitCard(apartment) {
    setText('[data-hmap-field="badge"]', apartment.badge);
    setText('[data-hmap-field="complexName"]', apartment.complexName);
    setText('[data-hmap-field="areaLine"]', apartment.legalDongLabel + " · " + apartment.areaGroup + " · " + (apartment.stationLabel || "역 접근성 확인 필요"));
    setText(
      '[data-hmap-field="profitSentence"]',
    ...
    setText('[data-hmap-field="gain"]', formatEok(apartment.estimatedGainManwon));
    setText('[data-hmap-field="gainRate"]', formatPercent(apartment.estimatedGainRate));
    setText('[data-hmap-field="latest"]', formatEok(apartment.latestTradePriceManwon));
    setText('[data-hmap-field="latestMeta"]', apartment.latestTradeDate + " · " + apartment.latestTradeArea);
    setText('[data-hmap-field="low"]', formatEok(apartment.fiveYearLowPriceManwon));
    setText('[data-hmap-field="riverNote"]', apartment.riverNote || "수변·생활권 메모 확인 필요");
    setText('[data-hmap-field="commuteNote"]', apartment.commuteNote || "통근 접근성 확인 필요");
    setText('[data-hmap-field="note"]', apartment.note);
  }
  ...
  function renderTable(sortKey) {
    var tbody = document.querySelector("[data-hmap-table-body]");
    if (!tbody) return;
  ...
  function renderBars(sortKey) {
    var list = document.querySelector("[data-hmap-bars]");
    if (!list) return;
    ...
    rows.forEach(function (row) {
      ...
      article.className = "hmap-bar-row" + (row.id === activeId ? " is-selected" : "");
      article.setAttribute("data-row-id", row.id);

      var label = document.createElement("div");
      label.className = "hmap-bar-row__label";
      ...
      bars.className = "hmap-bar-row__bars";
      var latest = document.createElement("span");
      latest.className = "hmap-bar hmap-bar--latest";
      ...
      low.className = "hmap-bar hmap-bar--low";
  ...
  function updateTabs() {
    document.querySelectorAll(".hmap-tab").forEach(function (button) {
