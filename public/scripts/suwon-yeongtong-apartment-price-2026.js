(function () {
  var dataEl = document.getElementById("sytap-data");
  if (!dataEl) return;
  ...
  function renderProfitCard(apartment) {
    setText('[data-sytap-field="badge"]', apartment.badge);
    setText('[data-sytap-field="complexName"]', apartment.complexName);
    setText('[data-sytap-field="areaMeta"]',
      apartment.legalDongLabel + " · " + apartment.areaGroup +
      (apartment.stationLabel ? " · " + apartment.stationLabel : "")
    );
    setText(
      '[data-sytap-field="profitSentence"]',
    ...
    setText('[data-sytap-field="gain"]', formatEok(apartment.estimatedGainManwon));
    setText('[data-sytap-field="gainRate"]', formatPercent(apartment.estimatedGainRate));
    setText('[data-sytap-field="latest"]', formatEok(apartment.latestTradePriceManwon));
    setText('[data-sytap-field="latestMeta"]', apartment.latestTradeDate + " · " + apartment.latestTradeArea);
    setText('[data-sytap-field="low"]', formatEok(apartment.fiveYearLowPriceManwon));
    setText('[data-sytap-field="jeonse"]', apartment.jeonseRatio ? formatPercent(apartment.jeonseRatio) + " 참고" : "확인 필요");
    setText('[data-sytap-field="note"]', apartment.note);

    var overlapBadge = document.querySelector(".sytap-badge--overlap");
    if (overlapBadge) overlapBadge.style.display = apartment.gwanggyoOverlap ? "" : "none";
  }

  function renderTable(sortKey) {
    var tbody = document.querySelector("[data-sytap-table-body]");
    if (!tbody) return;
    ...
    rows.forEach(function (row) {
      ...
      if (row.gwanggyoOverlap) {
        var overlapSpan = document.createElement("span");
        overlapSpan.className = "sytap-overlap-badge";
        overlapSpan.textContent = "광교 중복";
        nameTh.appendChild(overlapSpan);
      }
  ...
  function renderBars(sortKey) {
    var list = document.querySelector("[data-sytap-bars]");
    if (!list) return;
    ...
    rows.forEach(function (row) {
      ...
      article.className = "sytap-bar-row" + (row.id === activeId ? " is-selected" : "");
      article.setAttribute("data-row-id", row.id);

      var label = document.createElement("div");
      label.className = "sytap-bar-row__label";
      ...
      bars.className = "sytap-bar-row__bars";
      var latest = document.createElement("span");
      latest.className = "sytap-bar sytap-bar--latest";
      ...
      low.className = "sytap-bar sytap-bar--low";
  ...
  function updateTabs() {
    document.querySelectorAll(".sytap-tab").forEach(function (button) {
