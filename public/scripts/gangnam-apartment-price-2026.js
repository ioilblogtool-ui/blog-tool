(function () {
  var dataEl = document.getElementById("gnap-data");
  if (!dataEl) return;
  ...
  function renderProfitCard(apartment) {
    setText('[data-gnap-field="badge"]', apartment.badge);
    setText('[data-gnap-field="complexName"]', apartment.complexName);
    setText(
      '[data-gnap-field="areaLine"]',
      apartment.legalDongLabel + " · " + apartment.areaGroup + " · " + (apartment.stationLabel || "역 접근성 확인 필요")
    );
    setText(
      '[data-gnap-field="profitSentence"]',
    ...
    setText('[data-gnap-field="gain"]', formatEok(apartment.estimatedGainManwon));
    setText('[data-gnap-field="gainRate"]', formatPercent(apartment.estimatedGainRate));
    setText('[data-gnap-field="latest"]', formatEok(apartment.latestTradePriceManwon));
    setText('[data-gnap-field="latestMeta"]', apartment.latestTradeDate + " · " + apartment.latestTradeArea);
    setText('[data-gnap-field="low"]', formatEok(apartment.fiveYearLowPriceManwon));
    setText('[data-gnap-field="schoolNote"]', apartment.schoolNote || apartment.stationLabel || "생활권 메모 확인 필요");
    setText('[data-gnap-field="redevelopmentNote"]', apartment.redevelopmentNote || "재건축 기대 단지로 분류하지 않았습니다.");
    setText('[data-gnap-field="note"]', apartment.note);
  }
  ...
  function renderTable(sortKey) {
    var tbody = document.querySelector("[data-gnap-table-body]");
    if (!tbody) return;
  ...
  function renderBars(sortKey) {
    var list = document.querySelector("[data-gnap-bars]");
    if (!list) return;
    ...
    rows.forEach(function (row) {
      ...
      article.className = "gnap-bar-row" + (row.id === activeId ? " is-selected" : "");
      article.setAttribute("data-row-id", row.id);

      var label = document.createElement("div");
      label.className = "gnap-bar-row__label";
      ...
      bars.className = "gnap-bar-row__bars";
      var latest = document.createElement("span");
      latest.className = "gnap-bar gnap-bar--latest";
      ...
      low.className = "gnap-bar gnap-bar--low";
  ...
  function updateTabs() {
    document.querySelectorAll(".gnap-tab").forEach(function (button) {
