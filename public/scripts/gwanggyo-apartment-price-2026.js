(function () {
  var dataEl = document.getElementById("gdap-data");
  if (!dataEl) return;
  ...
  function renderProfitCard(apartment) {
    setText('[data-gdap-field="badge"]', apartment.badge);
    setText('[data-gdap-field="complexName"]', apartment.complexName);
    setText(
      '[data-gdap-field="areaMeta"]',
      apartment.cityLabel + " · " + apartment.areaGroup +
      (apartment.stationLabel ? " · " + apartment.stationLabel : "")
    );
    setText(
      '[data-gdap-field="profitSentence"]',
    ...
    setText('[data-gdap-field="gain"]', formatEok(apartment.estimatedGainManwon));
    setText('[data-gdap-field="gainRate"]', formatPercent(apartment.estimatedGainRate));
    setText('[data-gdap-field="latest"]', formatEok(apartment.latestTradePriceManwon));
    setText('[data-gdap-field="latestMeta"]', apartment.latestTradeDate + " · " + apartment.latestTradeArea);
    setText('[data-gdap-field="low"]', formatEok(apartment.fiveYearLowPriceManwon));
    setText('[data-gdap-field="jeonse"]', apartment.jeonseRatio ? formatPercent(apartment.jeonseRatio) + " 참고" : "확인 필요");
    setText('[data-gdap-field="areaNote"]', apartment.mainAreaLabel);
    setText('[data-gdap-field="note"]', apartment.note);
  }

  function renderTable(sortKey) {
    var tbody = document.querySelector("[data-gdap-table-body]");
    if (!tbody) return;
  ...
  function renderBars(sortKey) {
    var list = document.querySelector("[data-gdap-bars]");
    if (!list) return;
    ...
    rows.forEach(function (row) {
      ...
      article.className = "gdap-bar-row" + (row.id === activeId ? " is-selected" : "");
      article.setAttribute("data-row-id", row.id);

      var label = document.createElement("div");
      label.className = "gdap-bar-row__label";
      ...
      bars.className = "gdap-bar-row__bars";
      var latest = document.createElement("span");
      latest.className = "gdap-bar gdap-bar--latest";
      ...
      low.className = "gdap-bar gdap-bar--low";
  ...
  function renderAreaCards(apartment) {
    var cards = document.querySelectorAll("[data-gdap-area-cards] [data-area-group]");
  ...
  function updateTabs() {
    document.querySelectorAll(".gdap-tab").forEach(function (button) {
