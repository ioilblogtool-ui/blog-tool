...
(() => {
  ...
  const els = {
    annualSalary: root.querySelector("[data-gbc-annual-salary]"),
    monthlySalary: root.querySelector("[data-gbc-monthly-salary]"),
    monthlyHint: root.querySelector("[data-gbc-monthly-hint]"),
    resetMonthly: root.querySelector("[data-gbc-reset-monthly]"),
    taxMode: root.querySelector("[data-gbc-tax-mode]"),
    manualTaxRate: root.querySelector("[data-gbc-manual-tax-rate]"),
    bestNet: document.querySelector("[data-gbc-best-net]"),
    bestNetCompany: document.querySelector("[data-gbc-best-net-company]"),
    maxGap: document.querySelector("[data-gbc-max-gap]"),
    monthlyGap: document.querySelector("[data-gbc-monthly-gap]"),
    bestTotal: document.querySelector("[data-gbc-best-total]"),
    bestTotalCompany: document.querySelector("[data-gbc-best-total-company]"),
    resultTable: document.querySelector("[data-gbc-result-table]"),
    resetBtn: document.getElementById("gbcResetBtn"),
    copyBtn: document.getElementById("gbcCopyLinkBtn"),
  };
  ...
  function syncInputs() {
    ...
    companies.forEach((company) => {
      const input = state.companies[company.id];
      const mode = root.querySelector(`[data-gbc-mode="${company.id}"]`);
      const percent = root.querySelector(`[data-gbc-salary-percent="${company.id}"]`);
      const multiple = root.querySelector(`[data-gbc-monthly-multiple="${company.id}"]`);
      const fixed = root.querySelector(`[data-gbc-fixed-amount="${company.id}"]`);
      ...
      root.querySelectorAll(`[data-gbc-field-group="${company.id}"]`).forEach((field) => {
        field.hidden = field.dataset.gbcFieldMode !== input.mode;
      });
    });
  }
  ...
  function renderChart(results) {
    const canvas = document.getElementById("gbcCompareChart");
  ...
  function resetAll() {
    ...
    state.monthlySalary = Math.round(80_000_000 / 12);
  ...
  companies.forEach((company) => {
    root.querySelector(`[data-gbc-mode="${company.id}"]`)?.addEventListener("change", (event) => {
      state.companies[company.id].mode = event.target.value;
      update();
    });
    root.querySelector(`[data-gbc-salary-percent="${company.id}"]`)?.addEventListener("input", (event) => {
      state.companies[company.id].salaryPercent = parseNumber(event.target.value);
      update();
    });
    root.querySelector(`[data-gbc-monthly-multiple="${company.id}"]`)?.addEventListener("input", (event) => {
      state.companies[company.id].monthlyMultiple = parseNumber(event.target.value);
      update();
    });
    root.querySelector(`[data-gbc-fixed-amount="${company.id}"]`)?.addEventListener("input", (event) => {
