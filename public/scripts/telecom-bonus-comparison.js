...
(() => {
  const configEl = document.getElementById("tbcConfig");
  const root = document.querySelector("[data-tbc-calculator]");
  ...
  const state = {
    ...
    monthlySalary: Math.round(100_000_000 / 12),
  ...
  const els = {
    annualSalary: root.querySelector("[data-tbc-annual-salary]"),
    monthlySalary: root.querySelector("[data-tbc-monthly-salary]"),
    monthlyHint: root.querySelector("[data-tbc-monthly-hint]"),
    resetMonthly: root.querySelector("[data-tbc-reset-monthly]"),
    taxMode: root.querySelector("[data-tbc-tax-mode]"),
    manualTaxRate: root.querySelector("[data-tbc-manual-tax-rate]"),
    bestNet: document.querySelector("[data-tbc-best-net]"),
    bestNetCompany: document.querySelector("[data-tbc-best-net-company]"),
    maxGap: document.querySelector("[data-tbc-max-gap]"),
    monthlyGap: document.querySelector("[data-tbc-monthly-gap]"),
    bestTotal: document.querySelector("[data-tbc-best-total]"),
    bestTotalCompany: document.querySelector("[data-tbc-best-total-company]"),
    resultTable: document.querySelector("[data-tbc-result-table]"),
    resetBtn: document.getElementById("tbcResetBtn"),
    copyBtn: document.getElementById("tbcCopyLinkBtn"),
  };
  ...
  function syncInputs() {
    ...
    companies.forEach((company) => {
      const input = state.companies[company.id];
      const mode = root.querySelector(`[data-tbc-mode="${company.id}"]`);
      const percent = root.querySelector(`[data-tbc-salary-percent="${company.id}"]`);
      const multiple = root.querySelector(`[data-tbc-monthly-multiple="${company.id}"]`);
      const fixed = root.querySelector(`[data-tbc-fixed-amount="${company.id}"]`);
      ...
      root.querySelectorAll(`[data-tbc-field-group="${company.id}"]`).forEach((field) => {
        field.hidden = field.dataset.tbcFieldMode !== input.mode;
      });
    });
  }
  ...
  function renderResults(results) {
    ...
    if (els.resultTable) {
      els.resultTable.innerHTML = results.map((result) => {
  ...
  function renderChart(results) {
    const canvas = document.getElementById("tbcCompareChart");
    if (!canvas || !window.Chart) return;

    const labels = results.map((result) => result.company.shortName);
    const grossData = results.map((result) => result.grossBonus);
    const netData = results.map((result) => result.netBonus);
  ...
  function resetAll() {
    state.annualSalary = 100_000_000;
  ...
  companies.forEach((company) => {
    root.querySelector(`[data-tbc-mode="${company.id}"]`)?.addEventListener("change", (event) => {
      state.companies[company.id].mode = event.target.value;
      update();
    });
    root.querySelector(`[data-tbc-salary-percent="${company.id}"]`)?.addEventListener("input", (event) => {
      state.companies[company.id].salaryPercent = parseNumber(event.target.value);
      update();
    });
    root.querySelector(`[data-tbc-monthly-multiple="${company.id}"]`)?.addEventListener("input", (event) => {
      state.companies[company.id].monthlyMultiple = parseNumber(event.target.value);
      update();
    });
    root.querySelector(`[data-tbc-fixed-amount="${company.id}"]`)?.addEventListener("input", (event) => {
