...
function formatRatio(value) { return `${Number(value || 0).toFixed(2)}배`; }
function setText(id, value) { const el = $(id); if (el) el.textContent = value; }
function toNumber(input)    { return Number(input?.value || 0); }
function isChecked(input)   { return !!(input?.checked); }
function getMode()          { return modeCouple?.checked ? "COUPLE" : "SINGLE"; }
function getPayoutMode()    { return document.querySelector('input[name="payoutMode"]:checked')?.value || "ACTUAL"; }
...
function calculatePerson(annualSalary, year, payoutMode, scenarioCode, includePi, includeWelfare, welfareAmount) {
  ...
  return {
    annualSalary, baseSalary, psAmount,
    psImmediate:  psAmount * immediatePsRatio,
    psDeferred:   psAmount * deferredPsRatio,
    piAmount, welfareAmount: welfare, totalComp,
...
function renderScenarioChart(result) {
  ...
  const yearData = yearOptions.map((y) => {
    const eMode  = y.code === "2026" ? payoutMode : "SCENARIO";
    const s      = calculatePerson(selfAnnual,   y.code, eMode, scenarioCode, includePi, includeWelfare, welfareAmount);
    const sp     = mode === "COUPLE" ? calculatePerson(spouseAnnual, y.code, eMode, scenarioCode, includePi, includeWelfare, welfareAmount) : null;
    return {
      salary:  s.annualSalary  + (sp?.annualSalary  ?? 0),
      ps:      s.psAmount      + (sp?.psAmount      ?? 0),
      pi:      s.piAmount      + (sp?.piAmount      ?? 0),
      welfare: s.welfareAmount + (sp?.welfareAmount ?? 0),
    };
  });
...
function renderNetReport(result) {
  ...
  if (!enabled) { $("netReportList").innerHTML = ""; return; }
...
function renderScenarioYears(result) {
  ...
  $("scenarioYearGrid").innerHTML = yearOptions.map((yearItem) => {
    const eMode    = yearItem.code === "2026" ? payoutMode : "SCENARIO";
    const s        = calculatePerson(selfAnnual, yearItem.code, eMode, scenarioCode, includePi, includeWelfare, welfareAmount);
    const sp       = mode === "COUPLE" ? calculatePerson(spouseAnnual, yearItem.code, eMode, scenarioCode, includePi, includeWelfare, welfareAmount) : null;
    const total    = s.totalComp + (sp?.totalComp ?? 0);
    const psMulti  = getPsMultiplier(yearItem.code, eMode, scenarioCode);
    const note     = yearItem.code === "2026" && eMode === "ACTUAL" ? "실제 지급" : "시나리오";

    return `
      <article class="scenario-year-card${targetYearSelect.value === yearItem.code ? " is-active" : ""}">
        <p>${yearItem.label}</p>
        <strong>${formatKoreanAmount(total)}</strong>
        <span>PS ${formatRatio(psMulti)} / ${note}</span>
      </article>`;
  }).join("");
}
...
function resetPage() {
  ...
  const actualInput = $("payoutActual");
  if (actualInput) { actualInput.checked = true; actualInput.disabled = false; }
...
function bindRankSalary(select, input, hintId, sliderSync) {
  select.addEventListener("change", () => { syncRecommendedSalary(select, input, hintId); sliderSync?.(); render(); });
  input.addEventListener("input",   () => { updateSalaryHint(select, hintId); sliderSync?.(); render(); });
}
...
$("resetSkBonusBtn")?.addEventListener("click", () => { resetPage(); flashButton($("resetSkBonusBtn"), "초기화됨"); });
