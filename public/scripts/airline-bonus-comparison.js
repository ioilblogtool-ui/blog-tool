...
(() => {
  ...
  function syncInputs() {
    ...
    companies.forEach((company) => {
      const input = state.companies[company.id];
      const mode = root.querySelector(`[data-alb-mode="${company.id}"]`);
      const percent = root.querySelector(`[data-alb-salary-percent="${company.id}"]`);
      const multiple = root.querySelector(`[data-alb-monthly-multiple="${company.id}"]`);
      const fixed = root.querySelector(`[data-alb-fixed-amount="${company.id}"]`);
      ...
      root.querySelectorAll(`[data-alb-field-group="${company.id}"]`).forEach((field) => {
        field.hidden = field.dataset.albFieldMode !== input.mode;
      });
    });
  }
  ...
  function renderChart(results) {
    const canvas = document.getElementById("albCompareChart");
  ...
  function resetAll() {
    state.jobType = "ground";
    state.annualSalary = 70_000_000;
    state.monthlySalary = Math.round(70_000_000 / 12);
    state.monthlySalaryTouched = false;
    state.taxMode = "simple";
    state.manualTaxRate = 0.22;
    if (els.jobType) els.jobType.value = "ground";
    const groundJob = jobTypes.find((j) => j.id === "ground");
    if (els.jobHint && groundJob) els.jobHint.textContent = groundJob.avgSalaryHint;
    companies.forEach((company) => {
      state.companies[company.id] = {
        mode: company.defaultMode || "salaryPercent",
        salaryPercent: Number(company.defaultSalaryPercent || 0),
  ...
  companies.forEach((company) => {
    root.querySelector(`[data-alb-mode="${company.id}"]`)?.addEventListener("change", (event) => {
      state.companies[company.id].mode = event.target.value;
      update();
    });
    root.querySelector(`[data-alb-salary-percent="${company.id}"]`)?.addEventListener("input", (event) => {
      state.companies[company.id].salaryPercent = parseNumber(event.target.value);
      update();
    });
    root.querySelector(`[data-alb-monthly-multiple="${company.id}"]`)?.addEventListener("input", (event) => {
      state.companies[company.id].monthlyMultiple = parseNumber(event.target.value);
      update();
    });
    root.querySelector(`[data-alb-fixed-amount="${company.id}"]`)?.addEventListener("input", (event) => {
