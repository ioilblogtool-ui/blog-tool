(function () {
  var dataEl = document.getElementById("dtap-data");
  if (!dataEl) return;
  ...
  function renderProfitCard(apartment) {
    setText('[data-dtap-field="badge"]', apartment.badge);
    setText('[data-dtap-field="complexName"]', apartment.complexName);
    setText(
      '[data-dtap-field="profitSentence"]',
    ...
    setText('[data-dtap-field="gain"]', formatEok(apartment.estimatedGainManwon));
    setText('[data-dtap-field="gainRate"]', formatPercent(apartment.estimatedGainRate));
    setText('[data-dtap-field="latest"]', formatEok(apartment.latestTradePriceManwon));
    setText('[data-dtap-field="latestMeta"]', apartment.latestTradeDate + " · " + apartment.latestTradeArea);
    setText('[data-dtap-field="low"]', formatEok(apartment.fiveYearLowPriceManwon));
    setText('[data-dtap-field="note"]', apartment.note);
  }

  function renderTable(sortKey) {
    var tbody = document.querySelector("[data-dtap-table-body]");
    if (!tbody) return;
  ...
  function renderBars(sortKey) {
    var list = document.querySelector("[data-dtap-bars]");
    if (!list) return;
    ...
    rows.forEach(function (row) {
      ...
      article.className = "dtap-bar-row" + (row.id === activeId ? " is-selected" : "");
      article.setAttribute("data-row-id", row.id);

      var label = document.createElement("div");
      label.className = "dtap-bar-row__label";
      ...
      bars.className = "dtap-bar-row__bars";
      var latest = document.createElement("span");
      latest.className = "dtap-bar dtap-bar--latest";
      ...
      low.className = "dtap-bar dtap-bar--low";
  ...
  function updateTabs() {
    document.querySelectorAll(".dtap-tab").forEach(function (button) {
