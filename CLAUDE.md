...
새 페이지 생성 시 **반드시** 아래 공식을 적용한다. 나중에 수정하는 비용을 줄이기 위해 처음부터 CTR을 고려해 작성한다.

### 타이틀 공식 (페이지 유형별)

| 유형 | 공식 | 예시 |
|------|------|------|
| 계산기 | `{대상} 계산기 {연도} \| {핵심 결과} 바로 계산` | `퇴직금 계산기 2026 \| 세후 실수령액 바로 계산` |
| 비교 리포트 | `{A} vs {B} {연도} \| {핵심 수치} 한눈에 비교` | `골프 vs 헬스 비용 2026 \| 월 평균 얼마 차이날까` |
| 정보성 리포트 | `{주제} {연도} 완전 정리 \| {핵심 궁금증}` | `삼성 DS 성과급 2026 완전 정리 \| TAI·OPI 실수령` |
| 순위·비교표 | `{대상} {연도} 순위 \| {핵심 지표} 비교` | `건설사 연봉 순위 2026 \| 대형사 평균 연봉 비교` |

### 디스크립션 공식

```
{입력값} 입력하면 {결과값} 바로 계산. {부가 기능} 포함.
```

- 80~120자 유지
- 숫자·단위 포함 시 CTR 상승 (예: "세후 실수령", "월 평균 비용")
- 마지막에 기능 힌트 1개 추가 (예: "시뮬레이션 포함", "비교표 제공")

### 금지 표현 (CTR 낮아지는 패턴)

- `~를 위한 도구`, `~을 계산하는 페이지` — 기능 설명형 금지
- `환차손익`, `평균임금`, `TAI·OPI` 등 낯선 전문용어를 title 맨 앞에 배치 금지
- 연도 없는 타이틀 — 시의성 없으면 CTR 낮음, 반드시 `2026` 명시
- 50자 초과 타이틀 — 검색결과에서 잘림

### 체크리스트 (새 페이지 생성 시)

- [ ] 타이틀이 위 공식 중 하나를 따르는가?
- [ ] 타이틀에 연도(2026)가 포함되어 있는가?
- [ ] 디스크립션이 80자 이상인가?
- [ ] 검색자가 쓸 법한 일상 언어가 타이틀에 들어가 있는가?
