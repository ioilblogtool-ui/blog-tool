...
export const EVC_SEO_INTRO: string[] = [
  ...
  "이 계산기는 차량 가격·보조금·취득세·자동차세·연료비·정비비·보험료를 항목별로 분리해 5년·10년 TCO를 비교한다. 충전비는 완속·급속 비율을 직접 입력할 수 있어 아파트 거주자와 자가 충전 가능자 각각의 상황에 맞게 시뮬레이션할 수 있다.",
  "다만 정비비와 보험료는 추정값이며 차종·운전 이력·보험사별로 실제값이 다를 수 있다. 유가와 전기요금도 정책·계절·사업자에 따라 달라진다. 이 계산기는 참고용 시뮬레이션이며 실제 구매 전에는 해당 차종 견적서와 보험 다이렉트 비교를 함께 확인하길 권장한다.",
];
