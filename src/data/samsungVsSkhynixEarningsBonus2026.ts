...
export const SEVB_FAQ: FaqItem[] = [
  {
    ...
    answer:
      "직급과 시나리오에 따라 다릅니다. SK하이닉스는 PS 비율이 기준급의 24~70%(연도·시나리오별)로 책정되는 구조이고, 삼성전자는 사업부별 OPI 비율(DS 47% 등)에 DS 특별경영성과급(잠정합의안 기준 추가 10.5%)이 더해지는 구조입니다. 같은 기준급이라도 시나리오에 따라 우위가 바뀔 수 있어 이 페이지에서 직급·시나리오를 선택해 직접 비교해야 합니다.",
  },
  {
  ...
  {
    question: "삼성전자 DS 특별경영성과급 10.5%는 매년 나오나요?",
    answer:
      "아닙니다. DS 특별경영성과급은 사업 성과와 노사 협의에 따라 지급 여부와 규모가 달라지는 별도 재원으로, 반기마다 정기 지급되는 TAI(목표달성장려금)와는 다른 항목입니다. 이 페이지에 표시된 2026년 잠정합의안(총 12%, DS 특별경영성과급 10.5% 포함)은 2026년 노사 합의 기준이며, 향후 연도에 동일하게 반복된다고 가정할 수 없습니다.",
  },
];
...
export const SEVB_SEO_INTRO: string[] = [
  ...
  "성과급 환산은 각 회사의 기존 성과급 계산기에 쓰이는 보도 기준 비율을 그대로 사용합니다. SK하이닉스는 PS(생산성격려금) 비율로 2026년 보수적 24%부터 공격적 33%까지, 2027년에는 54~66%, 2028년에는 58~70%까지 시나리오별로 제시됩니다. 삼성전자는 사업부별 OPI 비율(예: DS부문 47%)에 2026년 잠정합의안 기준 DS 특별경영성과급 최대 10.5%가 더해지는 구조입니다.",
...
export const SEVB_SEO_CRITERIA: string[] = [
  ...
  "성과급 비율(PS/OPI/특별경영성과급)은 각 회사 성과급 계산기에 쓰이는 보도 기준 비율로, 실적 비교 섹션과 출처·시점이 다릅니다.",
