...
export const ITCP_SEO_INTRO = [
  "더 좋은 조건의 인터넷·TV 결합 상품을 발견했을 때 가장 먼저 막아서는 게 '지금 해지하면 위약금이 얼마나 나올까'라는 걱정입니다. 단순히 남은 약정 개월수만 보고 판단하면 실제 청구되는 금액과 차이가 커서, 갈아타는 게 손해인지 이득인지 헷갈리는 경우가 많습니다.",
  "위약금은 약정할인 반환금 하나로 끝나지 않습니다. 셋톱박스·공유기 같은 장비 할인 반환금, 설치 당시 무료로 받은 설치비, 가입 시 받은 사은품의 반환 조건까지 모두 더해져야 실제 해지 비용이 나옵니다. 이 계산기는 기존 상품 조건과 새로 가입할 상품 조건을 함께 입력해 예상 위약금, 신규 사은품을 반영한 순손익, 남은 약정기간 기준 손익분기점을 한 번에 계산합니다.",
  "결과에서 가장 중요한 숫자는 '손익분기점'입니다. 위약금과 신규 가입 혜택을 합산했을 때 몇 개월 안에 회수되는지를 보고, 그 기간이 남은 약정기간보다 짧다면 지금 갈아타는 것이 유리하다는 뜻입니다. 반대로 손익분기점이 너무 길면 기존 약정이 끝날 때까지 기다리는 편이 낫습니다.",
  "다만 통신사별 내부 할인반환금 산식과 장비 반납 상태, 프로모션 약관에 따라 실제 위약금은 이 계산기의 추정치와 다를 수 있습니다. 해지 신청 전 반드시 가입한 통신사 고객센터에 정확한 위약금 견적을 다시 확인하세요.",
];
