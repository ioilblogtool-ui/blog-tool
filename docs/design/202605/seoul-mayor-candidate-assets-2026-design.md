# 서울시장 후보 재산·부동산 비교 2026 설계 문서

> 기획 원문: `docs/plan/202605/seoul-mayor-candidate-assets-2026.md`  
> 작성일: 2026-05-31  
> 콘텐츠 유형: `/reports/` 선거 후보 비교 리포트  
> 구현 기준: 오세훈·정원오 후보 재산 신고액을 총재산, 부동산, 금융자산, 채무, 순자산 추정으로 분해하고 후보자 공개자료 조회 가이드까지 제공

---

## 1. 문서 개요

- 구현 대상: `서울시장 후보 재산·부동산 비교 2026`
- slug: `seoul-mayor-candidate-assets-2026`
- URL: `/reports/seoul-mayor-candidate-assets-2026/`
- 카테고리: 선거 데이터·후보 재산 비교
- 핵심 검색 의도: `서울시장 후보 재산`, `오세훈 정원오 재산`, `오세훈 재산`, `정원오 재산`, `서울시장 후보 부동산`
- 페이지 성격: 공식 후보자 공개자료를 기반으로 서울시장 후보의 자산 구조를 비교하는 데이터 리포트. 후보 지지·반대가 아니라 공개 신고액의 기준과 한계를 설명한다.
...
src/
  data/
    seoulMayorCandidateAssets2026.ts
    reports.ts
  pages/
    reports/
      seoul-mayor-candidate-assets-2026.astro

public/
  scripts/
    seoul-mayor-candidate-assets-2026.js
  og/
    reports/
      seoul-mayor-candidate-assets-2026.png

src/styles/scss/pages/
  _seoul-mayor-candidate-assets-2026.scss
```
...
- 최상위 클래스: `report-page smca-page`
- SCSS prefix: `smca-`
- 후보 2명 비교가 핵심이므로 `필터형 테이블`보다 `후보 비교 카드 + 비교표 + 자산 구성 막대`를 우선한다.
- 정치 기사 느낌보다 재산공개 대시보드 느낌으로 구성한다.
...
```astro
<main class="report-page smca-page" data-report="seoul-mayor-candidate-assets-2026">
  <Hero />
  <InfoNotice />
  <CandidateSnapshot />
  <ComparisonTable />
...
파일: `src/data/seoulMayorCandidateAssets2026.ts`
...
5. 오세훈 vs 정원오 재산 상세 비교표
6. 자산 구성 누적 막대
7. 부동산 신고액 비교
8. 예금·증권·금융자산 비교
9. 채무와 순자산 추정
10. 서울 부동산 공약과 후보 개인 재산 분리 안내
11. 서울시장 후보 공개자료 보는 법
12. 관련 리포트 CTA
13. FAQ
14. `SeoContent`
...
```text
이 페이지는 서울시장 후보자 공개자료를 비교하기 쉽게 정리한 참고 리포트입니다. 재산 신고액은 실제 현재 시세와 다를 수 있고, 후보자 본인 외 신고 대상 가족 재산이 포함될 수 있습니다. 후보 등록 이후 정정, 사퇴, 등록무효가 발생하면 비교표가 바뀔 수 있습니다.
```
...
- 1열 카드 스택
- 두 후보 카드 사이에 `vs` 장식 텍스트를 넣지 않는다. 비교는 숫자 카드로 충분하다.
...
- `.smca-metric-grid`
- `.smca-metric-card`
- `.smca-metric-card--primary`
- `.smca-metric-card--caution`
...
- `.smca-table-wrap { overflow-x: auto; }`
- 금액 셀 우측 정렬
- 계산값 행에는 `계산` 배지
- 확인되지 않은 값은 `확인 필요`
...
```astro
<div class="smca-composition-row">
  <div class="smca-composition-label">오세훈</div>
  <div class="smca-stacked-bar">
    <span class="smca-stacked-bar__segment smca-stacked-bar__segment--real-estate" style={`width:${ratio}%`} />
  </div>
</div>
```
...
- `서울시장 후보 부동산`, `오세훈 정원오 부동산` 검색 의도 대응
...
파일: `public/scripts/seoul-mayor-candidate-assets-2026.js`
...
```astro
<script id="smca-data" type="application/json" set:html={JSON.stringify(candidates)} />
...
```text
#smca-data
[data-smca-unit-toggle]
[data-smca-money]
[data-smca-candidate-card]
[data-smca-candidate-detail]
#smca-extra-candidates
#smca-extra-toggle
```
...
파일: `src/styles/scss/pages/_seoul-mayor-candidate-assets-2026.scss`
...
.smca-metric-card {}
.smca-table-wrap {}
.smca-comparison-table {}
.smca-composition-grid {}
.smca-stacked-bar {}
.smca-stacked-bar__segment {}
.smca-breakdown-grid {}
.smca-formula-card {}
.smca-policy-grid {}
.smca-guide-steps {}
...
1. 서울시장 후보 재산 공개자료를 보는 이유
2. 총재산·부동산·금융자산·채무를 나눠 봐야 하는 이유
3. 오세훈·정원오 후보 비교표와 자산 구성 막대를 읽는 법
4. 신고액의 한계와 선관위 원문 확인 필요성

inputPoints:

- 오세훈·정원오 후보의 총재산 신고액을 비교할 수 있습니다.
...
1. 서울시장 후보 재산은 어디서 확인할 수 있나요?
2. 오세훈·정원오 후보 재산은 어떤 기준인가요?
3. 부동산 신고액은 현재 아파트 시세인가요?
4. 후보 재산에는 배우자 재산도 포함되나요?
5. 채무가 있으면 총재산은 어떻게 봐야 하나요?
6. 후보 재산과 부동산 공약은 어떻게 연결해서 봐야 하나요?
...
- [ ] 후보 개인 재산과 부동산 공약을 단정적으로 연결하지 않는다.

## 15. 구현 순서

1. `src/data/seoulMayorCandidateAssets2026.ts` 생성
2. 후보 데이터와 메타·FAQ·관련 링크 작성
3. `src/pages/reports/seoul-mayor-candidate-assets-2026.astro` 생성
4. 후보 스냅샷 카드와 비교표 서버 렌더링
5. 자산 구성 막대와 부동산/금융/채무 섹션 작성
6. `public/scripts/seoul-mayor-candidate-assets-2026.js` 작성
7. SCSS 작성 및 `src/styles/app.scss` 등록
...
- 후보자 전과·납세·병역 요약 카드
- 후보별 공약 상세 비교
- 서울 25개 구 부동산 공약 연결 지도
- 후보 공개재산 전년 대비 변화
- 당선자 재산 공개 아카이브 전환
- 서울시장 후보 토론 후 이슈별 업데이트 박스

## 17. 최종 구현 방향

이 페이지는 `서울시장 후보 재산`, `오세훈 재산`, `정원오 재산`으로 들어오는 사용자를 받는 서울시장 선거 특화 리포트다. 사용자는 후보별 재산 규모를 알고 싶어 들어오지만, 페이지는 단순 승패식 비교가 아니라 총재산·부동산·금융자산·채무를 분리해서 읽는 기준을 제공해야 한다.

가장 중요한 것은 후보 개인 재산과 서울시 부동산 정책 공약을 섞어 단정하지 않는 것이다. 숫자는 크게 보여주되, 선관위 공개자료 기준·신고액 한계·자료 정정 가능성을 반복해서 안내한다. 비교계산소의 역할은 판단을 대신하는 것이 아니라, 공개된 정보를 안전하고 읽기 쉬운 형태로 정리하는 것이다.
