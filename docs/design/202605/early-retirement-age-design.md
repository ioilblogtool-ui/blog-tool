# 조기 은퇴 가능 나이 계산기 설계 문서

> 기획 원문: `docs/plan-docs/202605/early-retirement-age.md`  
> 작성일: 2026-05-05  
> 콘텐츠 유형: `/tools/` 계산기  
> 구현 기준: 필요 은퇴자산보다 “몇 살에 은퇴 가능한지”를 최상단에 표시

---

## 1. 문서 개요

- 구현 대상: `조기 은퇴 가능 나이 계산기`
- slug: `early-retirement-age`
- URL: `/tools/early-retirement-age/`
- 카테고리: 투자/연금/노후
- 핵심 검색 의도: 현재 소득·지출·자산으로 조기 은퇴 가능 나이 확인
- 계산 성격: 입력값 기반 시뮬레이션. 투자·은퇴 결정 권유가 아니다.
...
src/
  data/
    earlyRetirementAge.ts
  pages/
    tools/
      early-retirement-age.astro

public/
  scripts/
    early-retirement-age.js
  og/
    tools/
      early-retirement-age.png

src/styles/scss/pages/
  _early-retirement-age.scss
```
...
- `SimpleToolShell` 기반, `resultFirst={true}` 권장.
- 첫 화면에서 `만 48세 은퇴 가능`처럼 결과 후킹이 가장 강해야 한다.
- 입력값을 바꾸며 반복 계산하는 성격이 강하므로 결과 카드와 그래프가 빠르게 갱신되어야 한다.

```astro
<SimpleToolShell
  calculatorId="early-retirement-age"
  pageClass="era-page"
  resultFirst={true}
>
```
...
export interface EarlyRetirementProjectionPoint {
  month: number;
  age: number;
  assets: number;
  targetAssets: number;
...
export interface EarlyRetirementResult {
  ...
  monthsToRetirement: number | null;
  retirementAge: number | null;
  fireType: "lean" | "standard" | "fat" | "coast" | "barista";
  scenarios: EarlyRetirementScenario[];
  projection: EarlyRetirementProjectionPoint[];
}
```
...
월 저축액 = 월 순소득 - 월 지출
저축률 = 월 저축액 / 월 순소득 * 100
은퇴 후 순월생활비 = max(은퇴 후 월 생활비 - 월 연금 - 월 부수입, 0)
필요 은퇴자산 = 은퇴 후 연 생활비 / 안전 인출률 * (1 + 안전마진)
미래자산 = 현재자산 월복리 + 월 저축액 월복리 적립
은퇴 가능 나이 = 미래자산이 필요 은퇴자산 이상이 되는 최소 개월 수
```

1200개월(100년) 안에 목표 도달이 불가능하면 `null`로 처리한다.
...
2. 지출 10% 절감
3. 수익률 1%p 상승
4. 지출 10% 절감 + 수익률 1%p 상승

각 시나리오는 은퇴 가능 나이와 목표 도달 기간을 비교한다.

## 7. 페이지 IA

1. Hero
2. 시뮬레이션 고지 `InfoNotice`
3. 결과 요약 카드
4. 기본 입력
5. 고급 입력 접기/펼치기
6. 자산 성장 곡선
...
월 저축액이 0 이하이면 그래프 대신 지출 개선 안내를 먼저 표시한다.
...
(() => {
  ...
  function calculateMonthsToTarget(params) {}
  function createScenarios(input) {}
  ...
  restoreFromUrl();
  bindEvents();
  renderKpis(calculate(readInput()));
})();
```
...
- 투자/은퇴 계산기이므로 차분한 금융 톤.
- 경고 상태도 공포감을 주지 않고 `현재 조건에서는 목표 도달이 어렵습니다` 수준으로 표현한다.
- 그래프 높이는 모바일 260px, 데스크톱 340px 이상 확보한다.

## 12. SEO 설계

- title: `조기 은퇴 가능 나이 계산기: 나는 몇 살에 파이어족이 될 수 있을까?`
- description: `월 소득, 지출, 현재 자산, 투자수익률을 입력하면 목표 은퇴자산과 조기 은퇴 가능 나이를 계산합니다. 4% 룰, 안전 인출률, 시나리오별 은퇴 시점을 비교해보세요.`
- H1: `조기 은퇴 가능 나이 계산기`
- JSON-LD: `SoftwareApplication`, `FAQPage`

## 13. QA 체크리스트

- [ ] 월 저축액이 음수일 때 결과가 NaN이 되지 않음
- [ ] 현재 자산이 필요 자산 이상이면 즉시 은퇴 가능 처리
- [ ] 안전 인출률 3.0/3.5/4.0/4.5%별 필요 자산이 정확함
- [ ] 시나리오별 은퇴 나이가 기본보다 악화/개선 방향에 맞음
- [ ] 특정 투자수익률 보장 표현이 없음
- [ ] 국민연금 수령 전 공백 관련 고지가 있음
- [ ] 320px에서 결과 카드와 차트가 겹치지 않음
...
- 콘텐츠 유형: 계산기
- slug: early-retirement-age
- 주요 사용자: 30~50대 직장인, FIRE 관심 사용자
- 핵심 가치: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주요 리스크: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
...
- 목적: early-retirement-age 페이지에서 상단 요약 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.

#### 2. 입력 영역

- 목적: early-retirement-age 페이지에서 입력 영역 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 결과 영역 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 상세 해석 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 비교 표 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 모바일 화면 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 데스크톱 화면 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 데이터 출처 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 추정값 안내 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 FAQ 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 관련 링크 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 공유 기능 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 오류 처리 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 빈 상태 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 극단값 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 성능 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 접근성 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 SEO 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 QA 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 목적: early-retirement-age 페이지에서 배포 전 점검 블록이 사용자의 다음 판단을 돕도록 설계한다.
- 필수 표시: 자산, 저축률, 생활비를 바탕으로 은퇴 가능 시점을 추정한다.
- 주의 문구: 투자수익률과 물가상승률 가정에 따라 결과가 크게 흔들리므로 민감도 설명이 필요하다.
- 구현 메모: 텍스트, 숫자, 배지, 링크가 한 줄에 과도하게 몰리지 않도록 반응형 줄바꿈을 전제로 작성한다.
- 검수 기준: 모바일에서 블록의 첫 문장과 핵심 숫자가 동시에 이해되어야 한다.
...
- 작업 범위: early-retirement-age의 데이터 타입 정의 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
...
- 작업 범위: early-retirement-age의 기본값 구성 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 프리셋 작성 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 Astro 마크업 배치 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 data 속성 연결 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 계산 함수 분리 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 이벤트 바인딩 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 숫자 포맷터 작성 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 결과 문구 매핑 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 SCSS partial 작성 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 app.scss 등록 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 목록 데이터 등록 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 sitemap 등록 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 FAQ 렌더링 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 관련 링크 검증 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 모바일 확인 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 빌드 확인 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 콘솔 오류 확인 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 문구 검수 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
...
- 작업 범위: early-retirement-age의 추정 배지 확인 항목을 기존 Astro, SCSS, vanilla JS 패턴에 맞춰 구현한다.
- 완료 조건: 코드에서 하드코딩된 임시 문구, 미사용 변수, 빈 배열 placeholder가 남지 않는다.
- 검토 포인트: 데이터 파일, 페이지 마크업, 스크립트, 스타일, 등록 파일 사이의 slug가 모두 일치해야 한다.
- 실패 시 증상: 빌드 오류, 카드 미노출, 결과 미갱신, sitemap 누락, 모바일 overflow 중 하나로 드러날 가능성이 높다.
