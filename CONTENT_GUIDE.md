...
### SeoContent 콘텐츠 품질 기준 (AdSense Low value content 대응)

> **배경**: 구글 애드센스가 계산기 사이트에서 가장 자주 반려하는 사유가 **"Low value content"** 입니다.  
> 계산기 UI + 얇은 텍스트만 있으면 Google은 "도구만 있고 설명 없는 페이지"로 분류합니다.  
> `intro` 품질이 심사 통과 여부를 결정하는 핵심 변수입니다.
...
| 총 텍스트 | **600자 이상** |
| FAQ 항목 수 | **5개 이상** |
| FAQ 답변 길이 | 항목당 2~3문장 이상 |

#### intro 4단락 구조 (필수 순서)

```
단락 1 — 맥락 (Context)
  "이 계산기가 필요한 상황이 구체적으로 언제인가?"
  → 사용 시나리오 2~3가지 명시 (연봉 협상, 출산 준비, 이직 비교 등)

단락 2 — 메커니즘 (Mechanism)
  "어떤 원리로 계산하는가? 핵심 수식이나 기준은 무엇인가?"
  → 단순 기능 설명이 아니라 계산 방식의 '이유'를 설명

단락 3 — 해석 가이드 (How to read)
  "결과 숫자를 어떻게 읽어야 의미 있는 판단을 할 수 있나?"
  → 높은 값/낮은 값의 의미, 주의해야 할 구간, 다른 지표와 연결법

단락 4 — 한계와 주의사항 (Limitations)
  "이 계산기가 다루지 않는 것은 무엇인가? 실제와 다를 수 있는 이유는?"
  → 참고용임을 명시, 실제 확인이 필요한 공식 경로 안내
```

#### 나쁜 예 (Low value content로 거절됨)

```
intro={[
  "이 페이지는 기저귀 총비용을 월령별 사용량 기준으로 계산합니다.",  // 기능 설명만
  "장당 가격으로 맞춰 총 사용량을 계산합니다.",                      // 반복
]}
```
→ 두 단락 모두 기능 설명. 왜 필요한지, 결과를 어떻게 봐야 하는지 없음.
...
intro={[
  // 단락1 — 맥락
  "기저귀 비용은 출생 후 첫 1년 동안 매달 고정으로 나가는 주요 소모품 지출입니다.
   신생아 시기에는 하루 10~12장을 쓰다가 돌 무렵에는 5~7장으로 줄어드는
   월령별 변화를 반영해야 실제에 가까운 총비용이 나옵니다.",

  // 단락2 — 메커니즘
  "브랜드를 고를 때 '장당 가격'이 가장 공정한 비교 기준입니다.
   팩 구성이 달라도 장당 가격으로 맞추면 총 사용량 기준 차이가 명확해집니다.
   장당 500원 vs 350원이면 돌까지 총비용 차이가 20만~40만 원 이상 납니다.",

  // 단락3 — 해석 가이드
  "밤기저귀를 분리 사용하거나 대용량 묶음으로 구매하는 경우 실구매가가 달라집니다.
   멤버십 할인까지 감안하면 자주 구매하는 채널의 단가로 직접 입력하는 것이
   가장 정확합니다.",

  // 단락4 — 한계
  "이 계산기는 완전 기저귀 사용 기준이며 천기저귀 혼용, 기저귀 훈련 시작 시점
   차이는 반영되지 않습니다. 계산 기간과 브랜드별 가격을 실제에 맞게 조정해
   참고 자료로만 활용하세요.",
]}
```
...
- [ ] intro 단락이 4개 이상인가?
- [ ] 각 단락이 150자 이상인가?
- [ ] 단락1에 "누가, 언제 쓰는 계산기인지" 명시되어 있는가?
- [ ] 단락2에 계산 방식의 핵심 원리(수식·기준)가 설명되어 있는가?
- [ ] 단락3에 결과를 해석하는 방법이 있는가?
- [ ] 단락4에 한계와 공식 확인 권고가 있는가?
- [ ] FAQ가 5개 이상이고, 답변이 2문장 이상인가?
- [ ] related 링크가 3개 이상 연결되어 있는가?
...
### 7-A. 애드센스 심사 반려 사유와 대응

계산기 사이트는 **"Low value content"** 로 반려되는 경우가 가장 많습니다.  
반려 시 메일에서 확인할 수 있는 사유와 대응:

| 반려 사유 | 의미 | 대응 |
|----------|------|------|
| **Low value content** | 계산기 UI는 있으나 텍스트 설명이 얇음 | SeoContent intro 4단락 이상, FAQ 5개 이상으로 강화 |
| Insufficient content | 페이지 수 또는 전체 텍스트 볼륨 부족 | 페이지 수 늘리기, 기존 페이지 텍스트 보강 |
| Policy violation | 금지 콘텐츠 포함 | 해당 페이지 내용 검토·수정 |

**핵심 원칙**: Google은 계산기 UI만 있는 페이지를 "도구"로 보고 낮은 가치를 부여합니다.  
각 페이지가 "계산기 + 충분한 설명형 콘텐츠"의 조합이어야 합니다.

### 7-B. 심사 전 체크리스트

- [ ] About, Privacy, Terms, Contact, Disclaimer 페이지 모두 존재
- [ ] 모든 tool/report 페이지에 SeoContent 적용됨
- [ ] SeoContent intro가 4단락, 총 600자 이상
- [ ] FAQ 5개 이상
- [ ] ads.txt 등록 완료
- [ ] 공사중·빈 페이지 없음
- [ ] About 페이지에 운영자 이메일 명시
