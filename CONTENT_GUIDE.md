...
### 6-E. 외부 링크 / 사용자 다음 행동 CTA

계산기·리포트·콘텐츠 페이지에는 사용자가 결과 확인 후 다음 행동을 할 수 있도록
관련 외부 링크 또는 공식 참고 링크를 제한적으로 배치할 수 있다.
...
#### 원칙
- 본문 핵심 결과 이후에 배치한다
- 광고 클릭 유도처럼 보이는 문구는 사용하지 않는다
- 링크 성격을 명확히 표시한다
  - 예: `외부 서비스 링크입니다. 광고·제휴 아님.`
  - 예: `공식 사이트에서 자세히 보기`
  - 예: `제휴 링크가 포함될 수 있습니다.`
- 공식 출처 또는 신뢰 가능한 서비스 우선
- 한 페이지에 과도한 외부 링크를 두지 않는다
- 새 탭 링크는 `target="_blank" rel="noopener noreferrer"` 적용
- 제휴 링크는 별도 고지 및 `nofollow` 계열 rel 속성 적용

#### 권장 위치
- 계산기: 결과/해석 블록 아래, SeoContent 위
- 리포트: 인사이트 블록 뒤, SeoContent 앞
- 콘텐츠 페이지: 비교표/인사이트 뒤, 제휴 블록과 연속 배치 지양
---
